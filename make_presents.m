function [cost,gift_owned,gifted] =make_presents(state)
    ...
    odds=state(n_friends:end);% this variable says, when I can choose among different presents  (i.e. in the else below) which present I gift. It has not effect in the ''if'' case below. But in general I cannot predict a priori if it is needed or not.
    odds=odds/n_friends; % I want it normalised between [0,1]
    ...
    for i=2:n_friends
        ...
        if  isempty(indeces) % given the presents I have, all of them  would be spotted by the receiver
            ...
            indx=indeces(ceil(odds(i-1)*length(indeces)));% I pick one gift among the list of possibles( i.e. that do not clash with social relations). To do so I use pick one of the indices according to the number odds.  
