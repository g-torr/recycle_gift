...
n_neighbours=ceil(n_friends*rand(n_friends,1))% each node can have any number of neighbours,from an uniform distributon of integers till n_friends
neighbours=cell(n_friends,1);
for i=1:n_friends
    neighbours{i} = [randperm(n_friends,n_neighbours(i)),i]; %take randomly n_neighbours as friends and add itself
end
